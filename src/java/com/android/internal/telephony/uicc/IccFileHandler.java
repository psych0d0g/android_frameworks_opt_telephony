        ...
        try {
            ...
            case EVENT_GET_EF_LINEAR_RECORD_SIZE_DONE:
                ...
                if (data[0] == RESPONSE_DATA_TLV_TAG) {
                    ...
                    byte[] valueDescTag = parser
                        .getValueOfTag(REPOSNSE_DATA_TLV_FILE_DESC_TAG);
                    if (valueDescTag == null || valueDescTag.length != 5) {
                        throw new IccFileTypeMismatch();
                    }

                    byte[] valueSizeTag = parser
                        .getValueOfTag(RESPONSE_DATA_TLV_FILE_SIZE_TAG);
                    ...
                    logd("Tlv: fileSize:" + recordSize[1] + " recordSize:"
                        + recordSize[0] + " recordCount:" + recordSize[2]);
                } else {
                    if (TYPE_EF != data[RESPONSE_DATA_FILE_TYPE] ||
                        EF_TYPE_LINEAR_FIXED != data[RESPONSE_DATA_STRUCTURE]) {
                        throw new IccFileTypeMismatch();
             ...
             case EVENT_GET_RECORD_SIZE_DONE:
                ...
                if (data[0] == RESPONSE_DATA_TLV_TAG) {
                    ...
                    size = ((valueSizeTag[RESPONSE_DATA_TLV_FILE_SIZE_1] & 0xff) << 8)
                        + (valueSizeTag[RESPONSE_DATA_TLV_FILE_SIZE_2] & 0xff);
                    lc.mRecordSize = ((valueDescTag[RESPONSE_DATA_TLV_RECORD_SIZE_1] & 0xff) << 8)
                        + (valueDescTag[RESPONSE_DATA_TLV_RECORD_SIZE_2] & 0xff);
                    lc.mCountRecords = (valueDescTag[RESPONSE_DATA_TLV_RECORD_LENGTH] & 0xff);
                    logd("Tlv: fileSize:" + size + " recordSize:"
                        + lc.mRecordSize + " recordCount:" + lc.mCountRecords);
                } else {
                    if (TYPE_EF != data[RESPONSE_DATA_FILE_TYPE] ||
                        EF_TYPE_LINEAR_FIXED != data[RESPONSE_DATA_STRUCTURE]) {
                        throw new IccFileTypeMismatch();
