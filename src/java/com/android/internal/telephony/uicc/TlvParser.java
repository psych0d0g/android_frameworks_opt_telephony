...
/**
 * generic Tlv parser based on @see <a
 * href="http://mbyd.blogspot.co.at/2011/12/tlv-parser-part-2.html"/>
 **/
class TlvUnit {
	...
	public String toString() {
		return "0x" + Long.toHexString(tag) + "\t" + getLength() + "\t"
				+ stringValue();
	}
	...
	private String stringValue() {
		...
		for (final byte b : value) {
			hex.append("0x").append(HEXES.charAt((b & 0xF0) >> 4))
...
public class TlvParser {
	...
	public void parse(final byte[] tlvData) {
		...
		while (index + tagSize + lengthSize < tlvData.length) {
			...
			if ((index + length < 0) || (index + length > tlvData.length))
				break;
			list.add(new TlvUnit(tag, Arrays.copyOfRange(tlvData, index,
					(int) (index + length))));
			index += length;
		}
	}
	 ...
	 *             characters
	 */
	public void parseString(String tlvString) throws IllegalArgumentException {
		if (tlvString == null)
